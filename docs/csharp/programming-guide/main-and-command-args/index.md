...
- `Main` 在类或结构中声明。 `Main` 必须是[静态](../../language-reference/keywords/static.md)方法，不必为[公共](../../language-reference/keywords/public.md)方法。 （在前面的示例中，它获得的是[私有](../../language-reference/keywords/private.md)成员的默认访问权限）。封闭类或结构不一定要是静态的。
